...
import { Trophy, Plus, Save, RotateCcw, X, Check, Copy, Settings2 } from 'lucide-react'

interface GameBallInputProps {
    value: string
...
const DEFAULT_PRESETS: Preset[] = [
    { id: 'b1', label: '몰텐 BG4500', value: '몰텐 BG4500' },
    { id: 'b2', label: '스타 BB337', value: '스타 BB337' },
    { id: 'b3', label: '윌슨 에볼루션', value: '윌슨 에볼루션' },
    { id: 'b4', label: '나이키 엘리트', value: '나이키 엘리트' },
]

export default function GameBallInput({ value, onChange }: GameBallInputProps) {
    ...
    // Load presets
    useEffect(() => {
        const saved = localStorage.getItem('bdr_admin_gameball_presets')
        if (saved) {
    ...
    const savePresets = (newPresets: Preset[]) => {
        ...
        localStorage.setItem('bdr_admin_gameball_presets', JSON.stringify(newPresets))
    }
            ...
            <div className="flex gap-2">
                <div className="relative flex-1">
                    <input
                        ...
                        placeholder="예: 몰텐 BG4500"
                    />
                    <Trophy className="absolute left-3 top-2.5 w-4 h-4 text-gray-400" />
                </div>
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    placeholder="새 프리셋 이름 입력 (예: 스타 BB337)"
                                    autoFocus
                                    onKeyDown={(e) => {
                                        if (e.key === 'Enter') {
                                            e.preventDefault()
                            ...
                            <div className="space-y-1.5 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                                {presets.length === 0 ? (
                                    <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                                        <p className="text-xs text-gray-400">저장된 경기구 프리셋이 없습니다.</p>
                                    </div>
                                ) : (
                                    presets.map(preset => (
                                        <div
                                        ...
                                        >
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${value === preset.value ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400 group-hover:bg-blue-50 group-hover:text-blue-500'
                                                    ...
                                                    {value === preset.value ? <Check className="w-4 h-4" /> : <Trophy className="w-4 h-4" />}
                                                </div>
                                                <span className={`text-sm truncate ${value === preset.value ? 'font-bold text-gray-900' : 'text-gray-600 group-hover:text-gray-900'}`}>
                                                    {preset.label}
                                                </span>
                                            </div>
                                            <button
